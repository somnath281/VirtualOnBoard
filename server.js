...
io.on('connection', function(socket){
  ...
  var currentUser;
	...
	socket.on('MESSAGE_FROM_UNITY', function (_pack)
	{
		console.log('[INFO] Message received !!! ');
	  	
		var pack = JSON.parse(_pack);	

	    console.log('message from user# '+socket.id+":"+pack.message);
        
		//emit back to NetworkManager in Unity by client.js script
		io.sockets.emit('BROADCAST_TO_CLIENTS',socket.id,pack.message);
	...
	socket.on('LOGIN', function (_data)
	{
		...
		socket.emit("LOGIN_SUCCESS",currentUser.id,currentUser.name,currentUser.position,currentUser.avatar,currentUser.isAdmin);
         ...
         clients.forEach( function(i) {
			...
			{ 
		      //send to the client.js script
		      socket.emit('SPAWN_PLAYER',i.id,i.name,i.position,i.avatar,i.isAdmin);
		...
		socket.broadcast.emit('SPAWN_PLAYER',currentUser.id,currentUser.name,currentUser.position,currentUser.avatar,currentUser.isAdmin);
	...
	socket.on("AUDIO_MUTE", function (_data) {

		var data = JSON.parse(_data);	
		if(currentUser)
		{
			currentUser.isMute = !currentUser.isMute;
			//currentUser.isMute = data.isMute;
			console.log('[INFO] Total players: ' +currentUser.id +" ----- "+ currentUser.isMute);
			io.sockets.emit('USER_MUTE', currentUser.id,currentUser.isMute);
		}
